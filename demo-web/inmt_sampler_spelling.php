...
if (flock($fp, LOCK_EX))
{
	...
	$url = '172.28.1.3:6543/?source='.urlencode($source).'&prefix='.urlencode($prefix).'&learn='.urlencode($learn).'&beam_size='.urlencode($beam_size).'&length_norm='.urlencode($length_norm).'&coverage_norm='.urlencode($coverage_norm).'&alpha_norm='.urlencode($alpha_norm);
    $ch = curl_init();
